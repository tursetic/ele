...
export async function searchByAddress(params: {
  sido?: string; sigungu?: string; buldNm?: string; pageNo?: number; numOfRows?: string; signal?: AbortSignal;
  ...
  const query = new URLSearchParams({ serviceKey: SERVICE_KEY, numOfRows, pageNo: pageNo.toString() });
...
export async function fetchInspectionHistory(elevatorNo: string, pageNo: number = 1, signal?: AbortSignal): Promise<{ records: InspectionRecord[], totalCount: number }> {
  ...
  const url = `${BASE}/getElvtrInspctInqireM?serviceKey=${SERVICE_KEY}&elevator_no=${elevatorNo}`;
  const doc = await fetchXml(url, signal);
  const totalCount = parseInt(doc.querySelector('totalCount')?.textContent ?? '0', 10);
...
export function ensureKakaoReady(): Promise<void> {
  ...
  kakaoLoadPromise = new Promise<void>((resolve, reject) => {
    const TIMEOUT_MS = 5000; let settled = false;
    const timer = setTimeout(() => { if (settled) return; settled = true; kakaoLoadPromise = null; reject(new Error('Kakao SDK Load Timeout')); }, TIMEOUT_MS);
    if ((window as any).kakao && (window as any).kakao.maps?.load) { settled = true; clearTimeout(timer); (window as any).kakao.maps.load(() => resolve()); return; }
    ...
    script.referrerPolicy = 'unsafe-url'; script.async = true; script.defer = true;
    script.onload = () => {
      const kakao = (window as any).kakao;
      if (kakao && kakao.maps && typeof kakao.maps.load === 'function') {
        kakao.maps.load(() => { if (settled) return; settled = true; clearTimeout(timer); resolve(); });
      }
    };
    script.onerror = (err) => { if (settled) return; settled = true; clearTimeout(timer); kakaoLoadPromise = null; reject(new Error('Kakao CDN Network Error')); };
...
export function geocodeAddress(address: string, signal?: AbortSignal): Promise<[number, number] | null> {
  ...
  return ensureKakaoReady().then(() => {
    if (signal?.aborted) return null;
    return new Promise<[number, number] | null>((resolve) => {
      const kakao = (window as any).kakao; if (!kakao || !kakao.maps?.services) { resolve(null); return; }
      const geocoder = new kakao.maps.services.Geocoder();
      geocoder.addressSearch(address, (result: any[], status: string) => {
        if (signal?.aborted) { resolve(null); return; }
        if (status === 'OK' && result.length > 0) { resolve([parseFloat(result[0].y), parseFloat(result[0].x)]); } else { resolve(null); }
      });
    });
  }).catch(() => null);
}
...
export interface EleBuildingFeature {
  ...
  properties: { BULD_NM: string; ADDRESS: string; ELVTR_CNT: number; GROUND_FLOOR_CNT: number; UNDGRND_FLOOR_CNT: number; BULD_PRPOS_LCLAS_NM: string; BULD_PRPOS_SCLAS_NM: string; };
}
...
export async function fetchEleBuildings(bounds: { xmin: number; ymin: number; xmax: number; ymax: number; }, layerType: 'q' | 'limit' = 'q', signal?: AbortSignal) {
  ...
  const params = new URLSearchParams({ SERVICE: 'WFS', VERSION: '1.0.0', REQUEST: 'GetFeature', OUTPUTFORMAT: 'application/json', TYPENAME: typeName, BBOX: bboxParam, VIEWPARAMS: viewparamsParam });
...
export async function fetchInspectSafeList(elevatorNo: string, signal?: AbortSignal): Promise<any[]> {
  const url = `https://apis.data.go.kr/B553664/ElevatorInspectsafeService/getInspectsafeList?serviceKey=${SERVICE_KEY}&elevator_no=${elevatorNo}&numOfRows=100&pageNo=1`;
  try {
    const doc = await fetchXml(url, signal);
    return Array.from(doc.querySelectorAll('item')).map((item) => ({
      companyNm: getText(item, 'companyNm'), elvtrForm: getText(item, 'elvtrForm'), elvtrDetailForm: getText(item, 'elvtrDetailForm'),
      elvtrKindNm: getText(item, 'elvtrKindNm'), liveLoad: getText(item, 'liveLoad'), ratedSpeed: getText(item, 'ratedSpeed'),
      shuttleFloorCnt: getText(item, 'shuttleFloorCnt'), dispWords: getText(item, 'dispWords'), failCd: getText(item, 'failCd'), inspctDe: getText(item, 'inspctDe'),
    }));
  } catch (e) { console.error(e); return []; }
}

export async function fetchInspectFailList(failCd: string, signal?: AbortSignal): Promise<any[]> {
  const url = `https://apis.data.go.kr/B553664/ElevatorInspectsafeService/getInspectFailList?serviceKey=${SERVICE_KEY}&pageNo=1&numOfRows=100&fail_cd=${failCd}`;
  try {
    const doc = await fetchXml(url, signal);
    return Array.from(doc.querySelectorAll('item')).map((item) => ({
      failDesc: getText(item, 'failDesc'), failDescInspector: getText(item, 'failDescInspector'), standardArticle: getText(item, 'standardArticle'), standardTitle1: getText(item, 'standardTitle1'),
    }));
  } catch (e) { console.error(e); return []; }
}

export async function searchByIntegrationAddress(params: { addressQuery: string; pageNo?: number; signal?: AbortSignal; }): Promise<{ items: Elevator[], totalCount: number }> {
  const pageNo = params.pageNo ?? 1;
  const cacheKey = `integ:${params.addressQuery}:${pageNo}`;
  const cached = getCached(searchCache, cacheKey);
  if (cached) return cached;

  const query = new URLSearchParams({ serviceKey: SERVICE_KEY, numOfRows: '100', pageNo: pageNo.toString(), buld_address: params.addressQuery.trim() });
  const url = `https://apis.data.go.kr/B553664/ElevatorOperationService/getOperationInfoListV1?${query.toString()}`;
  const doc = await fetchXml(url, params.signal);
  const totalCount = parseInt(doc.querySelector('totalCount')?.textContent ?? '0', 10);
  
  const items = Array.from(doc.querySelectorAll('item')).map((item) => {
    const rawInstall = getText(item, 'installationDe');
    const rawFrst = getText(item, 'frstInstallationDe');
    const format8Digit = (s: string) => s && s.length === 8 ? `${s.slice(0,4)}-${s.slice(4,6)}-${s.slice(6,8)}` : s;

    return {
      elevatorNo: getText(item, 'elevatorNo'), buldNm: getText(item, 'buldNm'), address1: getText(item, 'address1'), address2: getText(item, 'address2'),
      elvtrAsignNo: getText(item, 'elvtrAsignNo'), elvtrDivNm: getText(item, 'elvtrDiv'),
      elvtrFormNm: `${getText(item, 'elvtrForm')}-${getText(item, 'elvtrDetailForm')}`, elvtrKindNm: getText(item, 'elvtrKindNm'),
      elvtrModel: getText(item, 'elvtrModel'), shuttleFloorCnt: getText(item, 'shuttleFloorCnt'), ratedSpeed: getText(item, 'ratedSpeed'),
      ratedCap: getText(item, 'ratedCap'), liveLoad: getText(item, 'liveLoad'), installationPlace: getText(item, 'installationPlace'),
      manufacturerName: getText(item, 'mnfcturCpnyNm'), elvtrStts: getText(item, 'elvtrSttsNm'), inspctInstt: getText(item, 'inspctInsttNm'),
      
      // 🎯 [완치 맵핑 추가] 유지관리업체 필드와 상하단 유효기간 연계를 위한 8자리 포맷 파싱 가드 강착
      mntCpnyNm: getText(item, 'companyNm'),
      applcBeDt: format8Digit(getText(item, 'applcBeDt')),
      applcEnDt: format8Digit(getText(item, 'applcEnDt')),
      
      frstInstallationDe: format8Digit(rawFrst), installationDe: format8Digit(rawInstall),
      lastResultNm: '정보없음', lastInspctDe: '', lastInspctKind: '', mrYn: 'N', subcntrCpny: '-'
    } as any;
  });
